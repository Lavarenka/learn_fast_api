...
from . import crud
from .chemas import Product, ProductCreate, ProductUpdate, ProductUpdatePartial
...
async def get_products(session: AsyncSession = Depends(db_helper.scored_session_dependency)):
    return await crud.get_products(session=session)


@router.post("/", response_model=Product, status_code=status.HTTP_201_CREATED)
async def create_product(product_in: ProductCreate,
                         session: AsyncSession = Depends(db_helper.scored_session_dependency)):
